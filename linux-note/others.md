...
- kprobe，上述的 tracepoint 和 ftrace 都是静态插桩，每次需要重新编译内核，效率低。kprobe则支持对内核进行动态插桩，即在内核运行过程中动态地增加或减少插桩点，并在插桩点处执行对应的插桩函数。由于kprobe支持在几乎所有内核代码处进行插桩，因此当我们希望观测内核中某处代码的执行情况时，就可直接在该处增加一个插桩点，并为其实现对应的插桩函数。
